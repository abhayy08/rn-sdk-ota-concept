...
public class Stallion {
  ...
  private static void mountNewStageBundle(String baseFolderPath) {
    ...
    if(stageTempHash != null && !stageTempHash.isEmpty()) {
      try {
        ...
        android.util.Log.e("StallionDebug", "mountNewStageBundle failed: " + e.getMessage(), e);
      }
    } else {
      android.util.Log.e("StallionDebug", "stageTempHash is empty, skipping mount");
    }
  }
  ...
  // ✅ CHANGED: hardcoded "/sdk.android.bundle" instead of ANDROID_BUNDLE_FILE_NAME constant
  private static String resolveBundlePath(String folderPath, String defaultBundlePath, String releaseHash, Boolean isProd) {
    String bundlePath = folderPath + StallionConfigConstants.UNZIP_FOLDER_NAME + "/sdk.android.bundle";
    if (new File(bundlePath).exists()) {
      return bundlePath;
    } else {
      if(isProd) {
        StallionSlotManager.rollbackProd(false, "Corrupted file not found" + folderPath);
  ...
  private static String getDefaultBundle(String defaultBundlePath) {
    return defaultBundlePath != null && !defaultBundlePath.isEmpty()
      ? defaultBundlePath
      : StallionConfigConstants.DEFAULT_JS_BUNDLE_LOCATION_BASE + "sdk.android.bundle";
  }
}